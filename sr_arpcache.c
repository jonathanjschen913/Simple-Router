...
void sr_send_icmp_packet(struct sr_instance *sr, uint8_t *packet, unsigned int len, char *iface, uint8_t type, uint8_t code);
void send_arp_req_packet_broadcast(struct sr_instance *sr, uint32_t target_ip);
...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
  ...
  /* using a while loop instead of a for loop as header suggests because I am clueless */
  while(req != NULL){
    next = req->next;
    /* helper mentioned in header */
...
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req) {
  ...
  if(difftime(timer, req->sent) > 1.0){
    if(req->times_sent >= 5){
      ...
      while(packet != NULL){
        sr_send_icmp_packet(sr, packet->buf, packet->len, packet->iface, 3, 1);
        packet = packet->next;
      }
      sr_arpreq_destroy(arp_cache, req);
    }
    else{
        send_arp_req_packet_broadcast(sr, req->ip);
        req->sent = timer;
        req->times_sent += 1;
    }
    return;
...
/* Send ICMP packet (type 3 - destination unreachable) */
void sr_send_icmp_packet(struct sr_instance *sr, uint8_t *packet, unsigned int len, char *iface, uint8_t type, uint8_t code) {
  assert(sr);
  assert(packet);
  assert(iface);

  /* Extract original packet headers */
  ...
  /* Copy first 28 bytes of original IP packet into ICMP data */
...
/* Send ARP request packet (broadcast) */
void send_arp_req_packet_broadcast(struct sr_instance *sr, uint32_t target_ip) {
  assert(sr);
  ...
  /* Find routing table entry for target IP */
  for (rt_entry = sr->routing_table; rt_entry != NULL; rt_entry = rt_entry->next) {
    if ((target_ip & rt_entry->mask.s_addr) == (rt_entry->dest.s_addr & rt_entry->mask.s_addr)) {
  ...
  if (!iface) {
    fprintf(stderr, "No interface found for target IP\n");
    return;
  }
  ...
  uint8_t zero_mac[ETHER_ADDR_LEN] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  memcpy(arp_hdr->ar_tha, zero_mac, ETHER_ADDR_LEN);
